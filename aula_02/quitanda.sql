...
-- ORDENANDO OS REGISTROS crescente
SELECT * FROM tb_produtos ORDER BY nome ASC;

-- ORDENANDO OS REGISTROS decrecente
SELECT * FROM tb_produtos ORDER BY nome DESC;

-- RETORNA VALORES ESPECIFICOS 
SELECT * FROM tb_produtos WHERE preco IN (5.00, 10.00, 15.00);

-- RETORNAR DADOS QUE ESTEJA ENTRE "5 E 15"
...
-- juntando informações de duas tabelas
SELECT nome, preco ,quantidade , tb_categorias.descricao
FROM tb_produtos
INNER JOIN tb_categorias
ON categoria_id = tb_categorias.id
ORDER BY nome; -- ordernando por nome

-- LEFT join, todos os produtos que estão na tabela a esquerda pegando todos os dados
SELECT nome, preco ,quantidade , tb_categorias.descricao
